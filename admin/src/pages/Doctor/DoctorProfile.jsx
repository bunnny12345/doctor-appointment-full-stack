  ...
  const updateProfile = async () => {
    try {
      ...
      if(data.success)
      {
        ...
        setIsEdit(false)
        getProfileData() //to update profiledata state
      } else{
    ...
    <div>
      <div className='flex flex-col gap-4 m-5'>
        ...
        <div className='flex-1 border border-stone-100 rounded-lg p-8 py-7 bg-white'>
          ...
          {
            isEdit ? <button onClick={updateProfile} className='px-4 py-1 border border-[#5f6fff] text-sm rounded-full mt-5 hover:bg-[#5f6fff] hover:text-white transition-all'>Save</button>
